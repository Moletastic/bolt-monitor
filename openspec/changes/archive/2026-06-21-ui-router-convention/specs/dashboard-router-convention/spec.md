...
Define when the Next.js client router APIs (`useRouter`, `usePathname`, `router.push`, `router.refresh`) are appropriate in the dashboard, and where `<Link>` and server actions should be used instead.
