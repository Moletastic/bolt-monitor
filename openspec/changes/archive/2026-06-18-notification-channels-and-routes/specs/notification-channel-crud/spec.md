...
The system SHALL reject channel create/update requests whose payload is invalid.

#### Scenario: Missing required field
- **WHEN** a request omits `name`, `type`, or `target`
- **THEN** the system returns `400 Bad Request` with `{ error: "<field> is required" }`

#### Scenario: Unknown channel type
- **WHEN** a request specifies `type` outside the allowed set
- **THEN** the system returns `400 Bad Request` with `{ error: "unsupported channel type" }`

#### Scenario: Type-specific credential missing
- **WHEN** a `telegram` channel is created with empty `config.botToken`, OR an `email` channel lacks `config.apiKey`/`config.fromEmail`, OR an `sms` channel lacks `config.accountSid`/`config.authToken`/`config.fromNumber`, OR a `pagerduty` channel lacks `config.routingKey`
- **THEN** the system returns `400 Bad Request` naming the missing credential
