...
### Requirement: Scheduler skips monitors on draft services
System SHALL NOT enqueue monitors where the parent service lifecycleState is "draft".

#### Scenario: Monitor on draft service is skipped
- **WHEN** scheduler evaluates a monitor where the parent service lifecycleState is "draft"
- **THEN** scheduler SHALL skip that monitor
- **AND** SHALL NOT send execution request to SQS

#### Scenario: Monitor on active service is processed
- **WHEN** scheduler evaluates a monitor where the parent service lifecycleState is "active"
- **THEN** scheduler SHALL process the monitor normally
