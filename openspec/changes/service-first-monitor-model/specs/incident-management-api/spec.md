...
System SHALL allow operators to inspect one incident directly and inspect incidents for one nested monitor.
...
- **WHEN** operator calls `GET /api/v1/services/{serviceId}/monitors/{monitorId}/incidents` for existing nested monitor
- **THEN** system returns incidents associated with that nested monitor
