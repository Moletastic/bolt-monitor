...
- **WHEN** nested monitor execution finishes
- **THEN** system persists a `CheckRun` record containing tenant, service, monitor, probe location, timing, and outcome data

### Requirement: System stores latest monitor status snapshot
System SHALL store a latest-status snapshot for each nested monitor.

#### Scenario: New result is processed
- **WHEN** system processes a completed execution result
- **THEN** it updates the nested monitor's current status snapshot with the latest derived state
...
- **WHEN** application needs historical run data and current state for one nested monitor
